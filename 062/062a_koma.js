...
const createScene = function () {
    ...
    if (1) {
        ...
        // 地面に摩擦係数、反射係数を設定する
        var groundAggregate = new BABYLON.PhysicsAggregate(groundMesh, BABYLON.PhysicsShapeType.BOX, { mass: 0, friction: 0.01, restitution:0.001}, scene);
    }
    ...
    function createKoma() {
        ...
        if (1) {
            ...
            trgAgg = new BABYLON.PhysicsAggregate(trgMesh, BABYLON.PhysicsShapeType.CONVEX_HULL, { mass: 100, restitution:0.05}, scene);
        ...
        if (0) {
            ...
            trgAgg = new BABYLON.PhysicsAggregate(trgMesh, BABYLON.PhysicsShapeType.CONVEX_HULL, { mass: 100, friction: 0.01, restitution:0.05}, scene);
        }
    ...
    // 左
    let [trgMesh, trgAgg]  = createKoma();
    trgMesh.physicsBody.disablePreStep = true;
    trgMesh.position = new BABYLON.Vector3(-3, 3, 3);
    trgMesh.rotationQuaternion = new BABYLON.Quaternion.FromEulerAngles(-0.2, 0, -0.2);
    trgMesh.physicsBody.disablePreStep = false;
    trgMesh.physicsBody.setAngularVelocity(new BABYLON.Vector3(0, 500, 0));
    let mat = new BABYLON.StandardMaterial("mat", scene);
    mat.emissiveColor = BABYLON.Color3.Red();
    trgMesh.material = mat;

    // 真ん中
    let [trgMesh2, trgAgg2]  = createKoma();
    trgMesh2.physicsBody.disablePreStep = true;
    trgMesh2.position = new BABYLON.Vector3(0, 3, 0);
    trgMesh2.rotationQuaternion = new BABYLON.Quaternion.FromEulerAngles(0, 0, 0);
    trgMesh2.physicsBody.disablePreStep = false;
    trgMesh2.physicsBody.setAngularVelocity(new BABYLON.Vector3(0, 500, 0));
    let mat2 = new BABYLON.StandardMaterial("mat", scene);
    mat2.emissiveColor = BABYLON.Color3.Green();
    trgMesh2.material = mat2;

    // 右
    let [trgMesh3, trgAgg3]  = createKoma();
    trgMesh3.physicsBody.disablePreStep = true;
    trgMesh3.position = new BABYLON.Vector3(3, 2, 3);
    trgMesh3.rotationQuaternion = new BABYLON.Quaternion.FromEulerAngles(0, 0, 0.2);
    trgMesh3.physicsBody.disablePreStep = false;
    let mat3 = new BABYLON.StandardMaterial("mat", scene);
    mat3.emissiveColor = BABYLON.Color3.Blue();
    trgMesh3.material = mat3;
    ...
    scene.registerAfterRender(function() {	
        if (map[" "]) {
            ...
            let v = 90;
            {
                ...
                let vdir = new BABYLON.Vector3(0 ,v, 0);
                vdir = vdir.applyRotationQuaternion(quat);
                trgMesh.physicsBody.applyAngularImpulse(vdir);
            }

            {
                let quat = trgMesh2.rotationQuaternion;
                let vdir = new BABYLON.Vector3(0 ,v, 0);
                vdir = vdir.applyRotationQuaternion(quat);
                trgMesh2.physicsBody.applyAngularImpulse(vdir);
            }

            {
                let quat = trgMesh3.rotationQuaternion;
                let vdir = new BABYLON.Vector3(0 ,v, 0);
                vdir = vdir.applyRotationQuaternion(quat);
                trgMesh3.physicsBody.applyAngularImpulse(vdir);
            }
        }
