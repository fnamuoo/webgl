...
async function loadDemBIN2(Dem, demw, demh, ampHigh) {
  ...
  // return {matrix, vzmax};
  // matrix のZ軸を入れ替え
  console.log("# loadDemBIN2()  swap z axis");
  console.log("#   matrix=", matrix);
  var matrix_ = [];
  const demh_ = demh - 1;
  for (let iz = 0; iz < demh; ++iz) {
  ...
  matrix = matrix_;
  // 物理系 heighfield 用に 軸を入れ替えたデータを用意する
  console.log("# loadDemBIN2()  swap axis for Ph")
  var momatrix = [];
  for (let ix = 0; ix < demw; ++ix) {
    momatrix.push([]);
    for (let iz = 0; iz < demh; ++iz) {
...
//  var demfpath = DembinI16v1;
//  var demfpath = DembinI16v2;
//  var {matrix, momatrix, vzmax} = await loadDemBIN2(demfpath, demw, demh, ampHigh); // ok
