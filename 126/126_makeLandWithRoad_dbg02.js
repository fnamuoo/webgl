...
const SCRIPT_URL22 = "../125/CourseData4.js";
...
// let skyboxType=3; // tropical/ grid
// let skyboxType=0; // 雪山
...
const courseInfoList = [
    ...
    [900711, 0, "KoenigsseeLuge_GERMANY_bank(難)", dbase+""],
...
//             if (myMesh.rotationQuaternion == null) {
//                 return;
//             }
// console.log("\nquat=", myMesh.rotationQuaternion);
//             let axisAngle = myMesh.rotationQuaternion.toAxisAngle;
// //console.log("\naxisAngle=", axisAngle);
...
//             const axis = BABYLON.Quaternion.RotationAxis(axisAngle.axis, axisAngle.angle);
...
console.log("stageType=",stageType);
            ...
            if ((stageType=='extrude_square_bottom_narrow')) {
                // 幅をやや狭く、複数車線用
                // 矩形（凹）で表示
                // // let gardW = typeof(metaStageInfo.gardW) !== 'undefined' ? metaStageInfo.gardW : 3.0;
            ...
            // 道路(plist3)に合わせてランダムな地形を作る
            {
                ...
                // let randCons = Array.from({ length: 2 }, () =>
                //                             Array.from({ length: nz }, () =>
                //                                        Array.from({ length: nx }, () => defaultValue)));
                // let randCons = Array.from({ length: nx }, () =>
                //                             Array.from({ length: nz }, () =>
                //                                        Array.from({ length: 2 }, () => defaultValue)));
                let randCons = Array.from(new Array(nx), () => { return Array.from(new Array(nz), () => new Array(2).fill(-1)); });

                if (0) {
                    // 高さに差をつけるテスト
                    const dR = 10; // 分布を割り当てる、plist3からの距離
                    const adjBankY = -1; // バンクをつけている分地面を下げる
                    ...
                    for (const p of plist3) {
                        ...
                        for (let iz=icz-dR ; iz <=icz+dR; ++iz) {
                            ...
                            for (let ix=icx-dR ; ix <=icx+dR; ++ix) {
                                ...
                                // let adjDistY = 0; // 平坦に
                                let adjDistY = Math.abs(ix-icx) < 6 ?  0 : -(ix-icx)*0.6; // X軸＋で下がるように
                                // let adjDistY = Math.abs(ix-icx) < 6 ?  0 : (ix-icx)*0.6; // X軸＋で上がるように
                                // let adjDistY = Math.abs(iz-icz) < 6 ?  0 : -(iz-icz)*0.6; // Z軸＋で下がるように
                                // let adjDistY = (Math.abs(ix-icx)+Math.abs(iz-icz)) < 6 ?  0 : -((ix-icx)+(iz-icz))*0.6; // X,Z軸＋で下がるように
                                if (randCons[ix][iz][0] == -1) {
                                    // 未設定：「高さ」「距離」を設定
                                    randCons[ix][iz][0] = p.y+adjBankY+adjDistY;
                                    randCons[ix][iz][1] = dist;
                                } else if (randCons[ix][iz][1] > dist) {
                                    // 設定済／より近い位置：より近い基準点の「高さ」「距離」を設定
                                    randCons[ix][iz][0] = p.y+adjBankY+adjDistY;
                                    randCons[ix][iz][1] = dist;
                                // } else if (randCons[ix][iz][1] == dist && randCons[ix][iz][0] > p.y+adjBankY+adjDistY) {
                                //     // 設定済／同じ距離の点：より低い「高さ」を設定
                                //     randCons[ix][iz][0] = p.y+adjBankY+adjDistY;
                                } else if (randCons[ix][iz][0] > p.y+adjBankY+adjDistY) {
                                    // 設定済／同じ距離の点：より低い「高さ」を設定
                                    randCons[ix][iz][0] = p.y+adjBankY+adjDistY;
                                    randCons[ix][iz][1] = dist;
                                }
                        ...
                        for (let iz=icz-dR ; iz <=icz+dR; ++iz) {
                            ...
                            for (let ix=icx-dR ; ix <=icx+dR; ++ix) {
                                ...
                                if (randCons[ix][iz][0] == -1) {
                                    ...
                                    randCons[ix][iz][0] = p.y+adjBankY;
                                    randCons[ix][iz][1] = dist;
                                // } else if (randCons[ix][iz][1] == dist && randCons[ix][iz][0] > p.y+adjBankY) {
                                //     // 設定済／同じ距離の点：より低い「高さ」を設定
                                //     randCons[ix][iz][0] = p.y+adjBankY;
...
// console.log("  j=",[jx,jz],"  i=",[ix,iz]);
                                if (ix < 0 || ix >= nx || iz < 0 || iz >= nz) {continue;}
                                // if (randCons[ix][iz][0] != null) {
                                if (randCons[ix][iz][0] != -1) {
                                    bfound = true;
                                    // if (randCons[jx][jz][0] != null) {
                ...
                if (0) {
                    // こちらの方法では偏りがでる：ループの最初にでてくるものほど優先して処理する
                    let rest = [];
                    for (let iz=0; iz < nz; ++iz) {
                        for (let ix=0; ix < nx; ++ix) {
                            // if (randCons[ix][iz][0] == null) {
...
// console.log("  j=",[jx,jz],"  i=",[ix,iz]);
                ...
                if (1) {
                    ...
                    function getShuffledRange(min, max) {
                        // 1. 指定範囲の配列を作成 (例: [1, 2, 3, 4, 5])
                        const arr = Array.from({ length: max - min + 1 }, (_, i) => min + i);
                        // 2. フィッシャー–イェーツのシャッフル
                        for (let i = arr.length - 1; i > 0; i--) {
                            const j = Math.floor(Math.random() * (i + 1)); // 0からiのランダムなインデックス
                            [arr[i], arr[arr[j]]] = [arr[j], arr[i]]; // 要素の入れ替え
                        }
                        return arr;
                    }

                    // 隣接する３セルにおいて、距離 randCons[][][1] が定数以上（道路から離れ）で、高さ randCons[][][0] を補間する
                    // 正確を期すなら、２状態（変更前、変更後）を用意すべきだが、面倒なので、ランダムにアクセスすることで１状態でもそれっぽくみせる
                    // >> 若干の変化、カドはとれてまるくなるけど、地形が変わるほどではない。
                    //    3x3だから？5x5ならもっと削れる？
                    //    ループ回数で100回せば、段々畑感は消えるけど崖っぷち感は残る
                    //    浸食な感じには見えない違和感はある
                    
                    let nloop = 100, thNxt = 5;
                    for (let iloop = 0; iloop < nloop; ++iloop) {
                        let zlist = getShuffledRange(1, nz-2);
                        for (let iz of zlist) {
                            let xlist = getShuffledRange(1, nx-2);
                            for (let ix of xlist) {
                                if (randCons[ix][iz][1] <= thNxt) {
                                    continue;
                                }
                                // randCons[ix][iz][0] = (randCons[ix-1][iz][0] + randCons[ix][iz-1][0] + randCons[ix+1][iz][0] + randCons[ix][iz+1][0])/4;
                                randCons[ix][iz][0] = (randCons[ix-1][iz-1][0] + randCons[ix-1][iz][0] + randCons[ix-1][iz+1][0] + randCons[ix][iz-1][0] + randCons[ix][iz+1][0] + randCons[ix+1][iz-1][0] + randCons[ix+1][iz][0] + randCons[ix+1][iz+1][0])/8;
                            }
                        }
                    }
                }
                ...
                if (1) {
                    ...
                    for (let iz=0; iz < nz; ++iz) {
                        ...
                        for (let ix=0; ix < nx; ++ix) {
                            ...
                            // y += amp*0.3;
                            // y += (Math.random()-0.5)*amp*0.3;
                            // y += nb.noise(8, 0.5, new BABYLON.Vector3(x,z,0), nbvzero, nbscale)*amp*0.3;
                    ...
                    meshAggInfo.push([trgMesh,null]);
                }

                if (0) {
                    // ノイズなし・道路からの距離の寄与（谷、尾根）
                    let rootPath = [];
                    for (let iz=0; iz < nz; ++iz) {
                        let z = (iz+nzmin)*gridsize;
                        let path = []
                        for (let ix=0; ix < nx; ++ix) {
                            ...
                            let y = randCons[ix][iz][0];
                            let amp = Math.max(randCons[ix][iz][1]-3,0);
                            // // y += Math.min((amp**2)*0.01, 50); // プラスなら谷
                            // y += amp*0.6; // プラスなら谷
                            y += -amp*0.6; // マイナスで尾根な感じに
                            path.push(new BABYLON.Vector3(x, y, z));
                        }
                        rootPath.push(path);
                    }
...
console.log("myMesh==null")
    ...
    let createMyMesh = function() {
        ...
        //         suspensionDamping:0.16, // 0.14,                 //[0-1] Damper force in percentage of suspensionForce
        //         suspensionAxisLocal:new BABYLON.Vector3(0,-1,0), //Direction of the spring
        //         axleAxisLocal:new BABYLON.Vector3(1,0,0),        //Axis the wheel spins around
        //         forwardAxisLocal:new BABYLON.Vector3(0,0,1),     //Forward direction of the wheel
        //         sideForcePositionRatio:0.1,              //[0-1]0 = wheel position, 1 = connection point 
        ...
        //         sideForce:6, // ski な感じ
        //         // sideForce:10, // power-drift & parallel turn
        //         // sideForce:12, // power-drift & parallel turn
        //         radius:0.2,
        //         rotationMultiplier:0.1,                   //How fast to spin the wheel
    ...
    let updateMyMesh = function() {
        ...
        //     // // let vbrake = (keyAction.brake) ? 1 : 0;
            ...
            // ..結果、横滑り（ドリフト）せずにグリップ走行になる
            {
                ...
                // // // 前方方向に少し力加える
                // // let vdirF = BABYLON.Vector3.Forward().applyRotationQuaternion(quat);
                // // let ampF=0.05;
                // // // vdirF.scaleInPlace(v*0.05+vec.length()*0.1);
                // // vdirF.scaleInPlace(vec.length()*0.01);
                // // myMesh._agg.body.applyImpulse(vdirF, myMesh.absolutePosition);
            }
