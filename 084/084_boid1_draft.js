...
class Boid {
    constructor(meshList, opt={}) {
        ...
        // this.idrng = typeof(opt.idrng) !== 'undefined' ? opt.idrng : [[0,n]];  // 全範囲を対象
        // 対象範囲の距離
        ...
        // this.rule1Distance = typeof(opt.rule1Distance) !== 'undefined' ? opt.rule1Distance : 50;
        // this.rule2Distance = typeof(opt.rule2Distance) !== 'undefined' ? opt.rule2Distance : 2;
        // this.rule3Distance = typeof(opt.rule3Distance) !== 'undefined' ? opt.rule3Distance : 10;
    ...
    updateOne(id) {
        // for (let idrng_ of this.idrng) {
        {
            // if (id < idrng_[0] || idrng_[1] <= id) {
            //     continue;
            // }
            ...
            // for (let i = idrng_[0]; i < idrng_[1]; ++i) {
...
// //console.log("other,", other, ", p=",other.position, ", v=",other_velocity);
//                 let d = BABYLON.Vector3.Distance(me.position, other.position);
//                 if (d < this.rule1Distance) {  // 集合・あつまる
...
// if (id == 1) { console.log("massCnt=",massCnt,", co=", cohesion); }
            }
            if (velCnt > 0) {
            ...
            // 集合の重心cohesion との距離2
// if (id == 1) { console.log("p=",mesh.position,", co=", cohesion); }
            let d2 = BABYLON.Vector3.DistanceSquared(mesh.position, cohesion);
// if (id == 1) { console.log("d2=",d2,", d2 ?> rule1", d2 > this.rule1Distance2, ", d2 ?> rurl3", d2 > this.rule3Distance2); }
...
// if (id == 1) { console.log("3 < 1"); }
                ...
                if (vcos < C5) {
                    ...
                    // 進行方向のみに加速する
                    // let vs = mesh.linImp * this.rule1Scale * BABYLON.Scalar.RandomRange(0.8, 1);
                    // vs *= (d2/this.rule31Distance2);
...
//if (id == 1) { console.log("2 < 3"); }
            ...
            // // 速さを反映？
            // let me_velocity = me.physicsBody.getLinearVelocity();
            // me_velocity.addInPlace(cohesion.scale(this.rule1Scale)
            //                        .add(separation.scale(this.rule2Scale))
            //                        .add(alignment.scale(this.rule3Scale)) );
            // me.physicsBody.setLinearVelocity(me_velocity);
            // // me.position.addInPlace(me.velocity.scale(deltaT));
        }
    }
    ...
    update() {
        ...
        // for (let idrng_ of this.idrng) {
        //     for (let id = idrng_[0]; id < idrng_[1]; ++id) {
        //         this.updateOne(id);
        //     }
        // }
    }
...
//        this.actModeDef = 'rand';
//        this.actModeDef = 'none';
        this.boid = new Boid([]);
    }
...
//console.log("\nZ2=", this.rangeZ2);
    ...
    createMeshAgg(n, opt) {
        ...
        for (let i = 0; i < n; ++i) {
            ...
            agg.body.setMotionType(BABYLON.PhysicsMotionType.DYNAMIC);  // ok?

            agg.body.setPrestepType(BABYLON.PhysicsPrestepType.TELEPORT); // ok?
            ...
            if (i < 3) {
                ...
                // let mesh2 = BABYLON.MeshBuilder.CreateBox("cube", { size: 1 }, this.scene);
                // mesh2.material = mesh.material.clone();
                // mesh2.material.alpha = 0.3;
                // this.mesh2List.push(mesh2);
            }
        }

        // if (1) {
        //     for (let [mesh,agg] of this.meshAggList) {
        //         console.log("\ngetLinearDamping=", mesh.physicsBody.getLinearDamping());
...
pz = BABYLON.Scalar.RandomRange(this.rangeZ[1]/2, this.rangeZ[1]);
        ...
        // var agg = new BABYLON.PhysicsAggregate(mesh, BABYLON.PhysicsShapeType.CONVEX_HULL, { mass: 10, restitution:0.01}, this.scene);
        mesh._agg = agg;
...
//console.log("iupdate=",this.iupdate);

        let i = -1;
        for (let [mesh,agg] of this.meshAggList) {
            ++i;
            // let mesh2 = null;
            // if (i < 3) {mesh2 = this.mesh2List[i];}
...
// console.log("reset Posture");
                ...
                // 目的地をランダム
                //  ok linearDamping=0.5, acc = 0.1 で巡回できるように
                //  ok ヨー（舵角）についてはOK
                //  xx ぶつかって真下を向くことがあり、壁（床）にぶつかりつづける
...
// console.log("reset GOAL/TARGET");
                        ...
                        // if (mesh2 != null) {
                        //     mesh2.position = mesh._targetPosi.clone();
                        // }
                    }
...
// console.log("reset Posture");
                    ...
                    // 進行方向のみに加速する / setLinearDamping(0.5) 時
                    // // agg.body.applyImpulse(BABYLON.Vector3.Forward(), mesh.absolutePosition);
                    let quat = mesh.rotationQuaternion;
                    let vdir = BABYLON.Vector3.Forward().applyRotationQuaternion(quat);
                    agg.body.applyImpulse(vdir.scale(mesh.linImp), mesh.absolutePosition);
                    // agg.body.applyImpulse(vdir, mesh.absolutePosition);
                }


            // } else if (mesh._actMode == 'rand_type1') {
            //     // 目的地をランダム
            //     //  ok linearDamping=0.5, acc = 0.1 で巡回できるように
            //     //  ok ヨー（舵角）についてはOK
            //     //  xx ぶつかって真下を向くことがあり、壁（床）にぶつかりつづける
            ...
            //         let vcross = vdir.cross(vtrg);
            //         // ２つのベクトルの角度の余弦値
            //         let vcos = vdirnrm.dot(vtrgnrm);

            //         // 舵角方向
            //         if (vcos < C10) {
            //             // setAngularDamping(0.5) 時
            //             if (vcross.y > 0) {
            //                 let quatRot = new BABYLON.Vector3(0, 0.01, 0);
            //                 agg.body.applyAngularImpulse(quatRot);
            //             } else {
            //                 let quatRot = new BABYLON.Vector3(0, -0.01, 0);
            //                 agg.body.applyAngularImpulse(quatRot);
...
var createScene = async function () {
    ...
    // let groundLevel = 13;
    // let groundLevel = -100;  // 深度に対するfogの確認のため
    // let groundLevel = -10;
    // let groundLevel = -100;  // 深度に対するfogの確認のため
    let groundLevel = -4;
...
//    let floorSize = 200, floorSize_ = floorSize/2;
...
//     meshAggManager2.init(opt);
    ...
    // enemyList[0]._trgMesh = displayCapsule;
