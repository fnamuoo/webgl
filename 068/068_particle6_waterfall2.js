...
var createScene = function () {
    ...
    if (1) {
    // 滝用の照明
    let light2 = new BABYLON.HemisphericLight("light", new BABYLON.Vector3(10, 36, -20), scene);
    light2.intensity = 0.3;
    let light3 = new BABYLON.HemisphericLight("light", new BABYLON.Vector3(-10, 36, -10), scene);
    ...
    if (1) {
        // 滝観賞用の足場
        let adjx = 0, adjz = -20, adjy = 1;
        let nbox = 7;
        for (let ibox = 0; ibox < nbox; ++ibox) {
            const trgMesh = BABYLON.MeshBuilder.CreateBox("target", { width: 3, height:0.01, depth:3 }, scene);
            trgMesh.material = new BABYLON.StandardMaterial("mat", scene);
            trgMesh.material.emissiveColor = BABYLON.Color3.Green();
            trgMesh.material.alpha = 0.2;
            ...
            trgMesh.position.x += adjx - 3*ibox;
            trgMesh.position.z += adjz;
            trgMesh.position.y += adjy + 3*ibox;
    ...
    // GroundPlayer用
//    wpFall2.material.diffuseTexture = new BABYLON.Texture("https://raw.githubusercontent.com/fnamuoo/webgl/main/068/textures/taki_Angel_Falls.png", scene);
    // ローカル環境用
    wpFall2.material.diffuseTexture = new BABYLON.Texture("textures/taki_Angel_Falls.png", scene);
    wpFall2.material.diffuseTexture.hasAlpha = true;
