...
export class Maze2 {
  ...
  setWall(posi: number[], idir: number, wallflag: number): void {
  ...
  create_from(sdata: string[]): void {
    ...
    for (let irow = 0; irow < this.nrow_; ++irow) {
      let sline = sdata.shift() as string;
  ...
  seekPath6(): [number, number[]] {
    ...
    while (candi.length > 0) {
      ...
      // // let vv = candi.shift() as {key: string]: number}; // 配列の先頭を取得
      let p = vv.rc;
      // ゴールに到達
      if (this.isGoal(p[0], p[1])) {
  ...
  seekPath7base(costMove: number[], costTurn: number[]): [number, number[]] {
    ...
    while (candi.length > 0) {
      console.assert(_iloop <= _nloop); // 無限ループ対策
      ++_iloop;
      let [p,idir8org]:any = candi.shift(); // 配列の先頭を取得／移動元の座標、方向、連続回数
      // ゴールに到達
      if (this.isGoal(p[0], p[1])) {
        if (costS2E < 0) {
