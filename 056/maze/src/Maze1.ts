...
function rc2idir(p1: number[], p2: number[]): number {
...
export class Maze1 {
  ...
  path3_: number[] = [];  // 深さ優先用 / seekPath3
  path6_: number[] = [];  // A* 用 / seekPath6
  deadend1_: number[] = []; // 行き止まり

  // 関数ポインタ
  seekPath: any;

  constructor(nrow: number =3, ncol: number =3) {
    this.nrow_ = nrow;
    this.ncol_ = ncol;
    this.nrow_1 = nrow-1;
    this.ncol_1 = ncol-1;
  ...
  protected init(defval: number = 1): number[][] {
  ...
  isWall(irow: number, icol: number): boolean {
    return (this.data_[irow][icol] == 1);
  }
  //壁の判定（領域判定つき：領域外は壁とする）
  isWall_s(irow: number, icol: number): boolean {
  ...
  enableDig_s(irow: number, icol: number): boolean {
  ...
  isStreet(irow: number, icol: number): boolean {
    return (this.data_[irow][icol] == 0);
  }
  isStreet_s(irow: number, icol: number): boolean {
    if ((irow < 0) || (icol < 0) || (irow >= this.nrow_) || (icol >= this.ncol_)) {
      return false;
    }
  ...
  create_dig(): void {
    ...
    while (hist.length > 0) {
      let posi = hist.pop() as number[]; // 型アサーションで undefined 対策
      irow = posi[0];
      icol = posi[1];
      {
  ...
  create_dig_2(pAperture: number, wStraight: number, wSlash: number): void {
    // 穴掘り法＋壁に穴開け＋直線・ななめの 汎用
    // ref) https://www.kerislab.jp/posts/2020-03-23-maze-generator/
    let nrow__ = this.nrow_ - 1;
    let ncol__ = this.ncol_ - 1;
    this.init(1);
  ...
  create_extendwall(): void {
    ...
    while (candi.length > 0) {
      let posi = candi.shift() as number[];
      let irow = posi[0];
      let icol = posi[1];
      {
        IDIRSH = shuffle2(IDIRSH);
  ...
  seekPath6(): [number, number[]] {
    ...
    let visited  = Array.from(new Array(this.nrow_), _ => new Array(this.ncol_).fill(false));
    visited[this.pStart_[0]][this.pStart_[1]] = true; // スタート位置
    function dist2goal(q: number[], pGoal: number[]): number {
    ...
    waitSort = nwaitSort;
    while (candi.length > 0) {
      let vv: any = candi.shift(); // 配列の先頭を取得
      let p = vv.rc;
      // ゴールに到達
      if (this.isGoal(p[0], p[1])) {
  ...
  protected seekSomePathBase(): number[][] {
    // 複数の経路をもとめる
    ...
    if (pathList.length > 0) {
      ...
      for (let i = 0; i < pathList.length-1; ++i) {
        for (let j = i+1; j < pathList.length; ++j) {
          let path1:number[] = pathList[i];
          let path2:number[] = pathList[j];
          let bswap = false;
  ...
  seekFarPoint(pStart: number[]): number[] {
    ...
    --nRest; // スタート地点を抜く
    while (candi.length > 0) {
      let p:any = candi.shift(); // 配列の先頭を取得
      // 未調査がない
      if (nRest == 0) {
        break;
      }
