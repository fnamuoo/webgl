...
const charPathList = ["../096/textures/pipo-charachip007_.png", // 男の子
                      "../093/textures/pipo-charachip008b.png", // 女の子
                      "../093/textures/pipo-charachip010.png", // ねこ
                      "../093/textures/pipo-charachip011b.png", // キング
                      "../093/textures/pipo-charachip012.png", // クイーン
                      "../093/textures/pipo-charachip017a.png", // メイド
                     ];
const charNameList = ["boy", "girl", "cat", "king", "queen", "maid"];
var myTextPath = charPathList[0];

const pathPanelList = [[533, 689, "../097/textures/einstein.png"], // アインシュタイン
                       [800, 719, "../097/textures/monogatari_inrou_mitokoumon.png"], // 水戸黄門
                       [648, 800, "../097/textures/nigaoe_kanu.png"],  // 関羽
                       [1013, 1177, "../097/textures/nigaoe_napoleon_horse.png"], // ナポレオン
                       [800, 800, "../097/textures/nigaoe_nasunoyoichi.png"],  // 那須与一
                       [711, 800, "../097/textures/nigaoe_sakamoto_ryouma.png"],  // 坂本龍馬
                       [724, 800, "../097/textures/nigaoe_shikoutei.png"],  // 秦の始皇帝
                       [778, 772, "../097/textures/nigaoe_syokatsu_koumei.png"],  // 諸葛孔明
                       [658, 800, "../097/textures/nigaoe_yagyuu_juubee.png"],  // 柳生十兵衛
                       [599, 696, "../097/textures/takeda_shingen.png"],  // 武田信玄
                      ]
...
// const charPathList = ["https://raw.githubusercontent.com/fnamuoo/webgl/main//096/textures/pipo-charachip007_.png", // 男の子
//                       "https://raw.githubusercontent.com/fnamuoo/webgl/main//093/textures/pipo-charachip008b.png", // 女の子
//                       "https://raw.githubusercontent.com/fnamuoo/webgl/main//093/textures/pipo-charachip010.png", // ねこ
//                       "https://raw.githubusercontent.com/fnamuoo/webgl/main//093/textures/pipo-charachip011b.png", // キング
//                       "https://raw.githubusercontent.com/fnamuoo/webgl/main//093/textures/pipo-charachip012.png", // クイーン
//                       "https://raw.githubusercontent.com/fnamuoo/webgl/main//093/textures/pipo-charachip017a.png", // メイド
//                      ];
// const charNameList = ["boy", "girl", "cat", "king", "queen", "maid"];
// var myTextPath = charPathList[0];
...
var createScene = async function () {
    ...
    var createStage = function(istage) {
        ...
        if(istage==0) {
            ...
            let [sw, sh, fpath] = pathPanelList[0];
        ...
        if(istage==1) {
            ...
            picr=10;
            let box1 = BABYLON.MeshBuilder.CreateBox("b", {width: 1, height:1, depth:1}, scene);
            let [sw, sh, fpath] = pathPanelList[1];
            sw=Math.ceil(sw/picr); sh=Math.ceil(sh/picr);
            let panel1 = BABYLON.MeshBuilder.CreatePlane("panel_1", {width:sw, height:sh}, scene);
            panel1.position = new BABYLON.Vector3(0, 1, 0);
            panel1.rotation = new BABYLON.Vector3(R90, 0, 0);
        ...
        if(istage==2) {
            ...
            picr=10;
            let box1 = BABYLON.MeshBuilder.CreateBox("b", {width: 1, height:1, depth:1}, scene);
            let [sw, sh, fpath] = pathPanelList[3];
            sw=Math.ceil(sw/picr); sh=Math.ceil(sh/picr);
            let panel1 = BABYLON.MeshBuilder.CreatePlane("panel_1", {width:sw, height:sh}, scene);
            panel1.position = new BABYLON.Vector3(0, 1, 0);
            panel1.rotation = new BABYLON.Vector3(R90, 0, 0);
        ...
        if(istage==3) {
            ...
            picr=10;
            let box1 = BABYLON.MeshBuilder.CreateBox("b", {width: 1, height:1, depth:1}, scene);
            let idxlist = [2, 4, 5, 6, 8];
            let ii = -1;
            let whList = [];
            for (let idx of idxlist) {
                ++ii;
                let [sw, sh, fpath] = pathPanelList[idx];
                sw=Math.ceil(sw/picr); sh=Math.ceil(sh/picr);
                let panel1 = BABYLON.MeshBuilder.CreatePlane("panel_2"+ii, {width:sw, height:sh}, scene);
                panel1.position = new BABYLON.Vector3(0, sh/2+1, 10*ii);
                panel1.material = new BABYLON.StandardMaterial("mat1org", scene);
                panel1.material.diffuseTexture = new BABYLON.Texture(fpath, scene);
                panel1.material.diffuseTexture.hasAlpha = true;
	        panel1.material.emissiveColor = new BABYLON.Color3.White();
                ...
                let particleNb=sw*sh;
                SPS.addShape(box1, particleNb);                
                whList.push([ii,sw,sh]);
            }
            spsMesh = SPS.buildMesh(); // finally builds and displays the SPS mesh
            SPS.initParticles = function() {
                let i=0;
                for (let [ii,sw,sh] of whList) {
                    let sw_ = sw/2, sh_ = sh/2, nj=sw*sh, jz=ii*10;
    ...
    let weaponLabel = ["None(board only)",
                       "Big Ball",
                       "Lance",
                       "A rotating Ellipse",
                       "Sword",
    ...
    var createMyMesh = function(imesh) {
	    ...
	    wpMesh.material.alpha = 0.7;
            ...
            for (let iwp=0; iwp<nwp; ++iwp) {
                wpMeshList[iwp]._anima = [zSlide];
                wpMeshList[iwp]._nframe = nframe;
            }
            actMyMesh = function() {
    ...
    let guiCharaRect = new BABYLON.GUI.Rectangle(); 
    {
        ...
        {
            let button = BABYLON.GUI.Button.CreateSimpleButton("b2", "Wrapon");
            button.width = "140px";
            button.height = "40px";
    ...
    var showCtrlChooseWeapon = function() {
        if (guiWeaponRect.isVisible) {
            ...
            weaponBtnList[imesh].background = "DarkGray";
