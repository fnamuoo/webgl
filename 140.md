...
正直見た目だけならクランク側を回転させて、それに合わせてコンロッド・ピストンを動かすのが簡単です。
ですが、ピストン側を前後に動かすことに こだわります。

ピストン側を前後に動かしてクランクを回転運動にするため、
クランク・コンロッド・ピストンが直線に並んだ状態（機械工学的にいうところの「死点でのロック」）に注意する必要があります。
そこで、SL（蒸気機関車）のように２つのクランク－コンロッド－ピストンの機構を並べて、９０度ずらしてクランクと結合します。
...
メッシュ位置を計算する関数を作っておけば、次の物理構造のときの初期位置に使えるので無駄にならないです。
...
![](140/pic/140_ss_11.jpg)  
*メッシュ（点）でのスライダクランク機構*

車輪（動輪）やピストン、スライダをつけて装飾したら、以下のような感じになります。

![](140/pic/140_ss_12x2.gif)  
...
スライダクランク機構に必須なメッシュおよびアンカー（固定部）用のボディを用意します。
ここで車輪は見た目だけのメッシュに留めて、クランクに固定（親子指定）しているだけです。

動作確認として、
「無重力環境下で完全に静止していること」、
「クランク側を回転させてスムーズに動くこと」になっているかを確認します。

![](140/pic/140_ss_13x1.gif)  
...
![](140/pic/140_ss_14x1.gif)  
*物理形状のスライダクランク機構（ピストン動作）*

### 重力環境下で三輪車を走らせてみる

上述までで、スムーズに動くことが確認できたので、車輪を物理化（体積、質量、摩擦係数）を付けて、
ボディも固定化(mass=0)を解除して質量を指定(mass=1)し、可動できるようにします。
重力を有効にして、質量・摩擦係数・ピストンの力を調整して、なんとか直進することができました。

![](140/pic/140_ss_15x1.gif)  
*重力下でのテスト走行*
...
スピードが出てくるとスライダクランク機構の動きが影響するようで、三輪車がフラフラします。
気になる方は上記の Playground のリンクでご確認ください。

## まとめ・雑感

三輪車のテスト走行で満足してしまいました。
そして４カメラを実装して気力が尽きました。

物理演算で安定な走行を目指して「パラメータ調整の沼」にハマるのは勘弁願いたいところです。
SLを走らせるならメッシュで十分な気がする、いやそうすべきですね。
ピストン駆動でもクランク駆動でも、見た目は一緒だし。

------------------------------

前の記事：[Babylon.js で物理演算(havok)：中に重りのある球形を転がしてみる](139.md)

次の記事：[Babylon.js + Havokで二足歩行を安定させる：クランク機構を「壊れない形で作る3ステップ」](141.md)
