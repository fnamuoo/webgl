...
![](127/pic/127_ss_22x2.gif)  
*同一ラインを追いかけっこ（２倍速）*

![](127/pic/127_ss_29x2.gif)  
*電車［台車付］（車両長め）（２倍速）*
...
Path3Dに対して getPointAt() で [0, 1]の範囲の値を渡すと、それに応じた座標値を返してくれる便利な関数があるのでこれを使います。
また、姿勢・向きについても接線方向を取得する関数 getTangentAt()があるのでこれを利用してレンダリングを変更します。
...
接線方向に向きを合わせているだけなので、車両が長く／急カーブがあると、動きがおかしくなります。
より正確な動きにしたいなら台車（車輪）を考慮して、その上に車両の乗せるべきかと思います。
ここでは厳密さを求めない上に、コスパ（実装コストや処理に対する見た目の良さ）は今一つな感じがするのでこのまま進めることにします。

![](127/pic/127_ss_25x2.gif)  
*電車（単線）（２倍速）*
...
![](127/pic/127_ss_27x2.gif)  
*電車（複線）／複数車両（２倍速）*

### 例：電車［台車付］（単線）

上述までは、「簡単な車両」（１点のみで位置と向きを表す）でした。
ここでは台車・車輪と車両をわけて車両Ｎに対し、車輪位置をＮ＋１を使い、車両位置は隣接する車輪の中間位置とします。
計算量が少々増えるので、列車の車両や本数を増やして描画させようとするとちょっと重いかもです。

下記、例では、車両をやや長めにして挙動を確認しています。
車両を長く／急カーブでも違和感がかなり軽減されたと思います。
急カーブを通過するときに車輪の位置が若干おかしい（車両が詰まる）感じがして、車両がめり込むのはご愛嬌です。

車両の向きを台車の２点から Math.atan2() で求めていますが平面上なので良しとします。
レールにアップダウン（ピッチ角）やバンク（車両のロール）があると破綻します。
台車の中間地点の getTangentAt() を使えば良いかもしれません。ここではそこまで求めないのでこれ(atan2)で良しとします。


![](127/pic/127_ss_28x2.gif)  
*電車［台車付］（２倍速）*
...
処理を簡略化しているために、よく見ると動きに対する違和感があります。
車両が長くなったり、急カーブがあると顕在化します。
精緻さ／正確さを求めないのであれば十分な出来と思います。
...
前の記事：[Babylon.js で物理演算(havok)：ＡＩに頼りつつ「道路に合わせた地形」を作成する](126.md)

次の記事：[Babylon.js ：簡易な鉄道シミュレーション（ゆりかもめ）](128.md)
