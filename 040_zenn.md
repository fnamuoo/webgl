...
といっても、実際にドローンは持ってないし、ドローンシミュレータを触ったこともないので、「なんちゃってドローン」です。
...
- キーボード入力との両立
  ...
  - どちらでも有効なように作ればよかったのですが、イベント処理が面倒そうだったので、どちらか一方（ゲームパッドかキーボードか）に絞ってます。
  - キーボードでもゲームパッド同様に操作できるようにしてます。

### スロープトイ拡張

以前の「スロープトイ」では、こぢんまりしたパーツばかりでスケールアップすると部品数が増えることになります。
