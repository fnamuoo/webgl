// Babylon.js で物理演算(havok)：SpringConstraintで太陽系(2)
...
var springEx1_a3 = function (scene, opt={}) {
    ...
    let diam0 = typeof(opt.diam0) !== 'undefined' ? opt.diam0 : 2;
    let p1x = typeof(opt.p1x) !== 'undefined' ? opt.p1x : 0;
    let p1y = typeof(opt.p1y) !== 'undefined' ? opt.p1y : 0;
    let p1z = typeof(opt.p1z) !== 'undefined' ? opt.p1z : 0;
    ...
    let p2x = typeof(opt.p2x) !== 'undefined' ? opt.p2x : 8;
    let p2y = typeof(opt.p2y) !== 'undefined' ? opt.p2y : 0;
    let p2z = typeof(opt.p2z) !== 'undefined' ? opt.p2z : 0;
    ...
    if (sp1 == null) {
        sp1 = BABYLON.MeshBuilder.CreateSphere("target", { diameter: diam0, segments: 8 }, scene);
        sp1.position.set(p1x+adjx, p1y+adjy, p1z+adjz);
        setAutoMat(sp1, "grid");
        agg1 = new BABYLON.PhysicsAggregate(sp1, BABYLON.PhysicsShapeType.SPHERE, { mass: 0, restitution:1}, scene);
    }

    if (sp2 == null) {
        sp2 = BABYLON.MeshBuilder.CreateSphere("target", { diameter: diam, segments: 8 }, scene);
        sp2.position.set(p2x+adjx, p2y+adjy, p2z+adjz);
        if (txt == "") {
            setAutoMat(sp2);
        } else {
        ...
        agg2 = new BABYLON.PhysicsAggregate(sp2, BABYLON.PhysicsShapeType.SPHERE, { mass: mass, restitution:1, friction:0.001}, scene);
    }
...
//    useViewer = true;
    ...
    planetMesh = [];
    {
        ...
        trgPhysList = [];
        {
            let meshSun, aggSun, meshPlanet, aggPlanet, meshEarth, aggEarth, mesh1, agg1;

            // 0:水星
            opt = {adjx:0, adjz:0,
                   txt:txt_mercury, col:[0,1,1],
                   diam:1, p2x:2, mass:10,
            ...
            // 7:彗星(2)
            opt = {txt:txt_comet2, col:[1,1,1],
                   diam0: 0.01, p1x:3,
                   diam:0.6, p2x:65, mass:3,
                   minDist:1, maxDist:2, stiff:0.5, damping:0,
                   motorTrg:1, motorMFo:5, appImp:10,
                   sp1:meshSun, agg1:aggSun};
            [meshSun, aggSun, meshPlanet, aggPlanet] = springEx1_a3(scene, opt);
            opt = {sp2:meshPlanet, agg2:aggPlanet,
                   diam0: 0.01, p1x:12,
                   minDist:1, maxDist:4, stiff:0.5, damping:0,
                   motorTrg:1, motorMFo:5, appImp:10};
            [mesh1, agg1, meshPlanet, aggPlanet] = springEx1_a3(scene, opt);
            planetMesh.push(meshPlanet);
            opt2 = {py:0.3, life:2};
            addBubble(meshPlanet, opt2);
