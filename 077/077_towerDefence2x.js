...
var createScene = function () {
	...
	ground.material.alpha = 0.7;
        ...
        // スタート地点のパネル
        {
            ...
            let [x,z] = ixz2xz(ix,iz);
            mesh.position = new BABYLON.Vector3(x, 0.01, z);
        ...
        for (let iz = 0; iz < nz; ++iz) {
            ...
            for (let ix = 0; ix < nx; ++ix) {
                ...
                if (maze.data_[iz][ix]) { // 壁
                    let mesh = BABYLON.MeshBuilder.CreateBox("wall", { width:wL, height:wH, depth:wL }, scene);
    ...
    let maxAgents = 10, iniAgents = 10, nAgents = maxAgents;
    var agentParams = {
    ...
    let setDest0_ = function(agID) {
        // 初期位置：スタート地点の下にテレポート
        [iz, ix] = maze.pStart_;
        let [x,z] = ixz2xz(ix,iz);
        p = new BABYLON.Vector3(x, -wH*2, z);
        crowd.agentTeleport(agID, navigationPlugin.getClosestPoint(p));
    }
    ...
    let createCannonMesh = function(posi) {
        let viewMesh = BABYLON.MeshBuilder.CreateSphere("cannon", { diameter: 1, segments: 8 }, scene);
        viewMesh.position = posi;
        // 衝突判定用メッシュ
        let rangeMesh = BABYLON.MeshBuilder.CreateSphere("range", { diameter: 4, segments: 8 }, scene);
        rangeMesh.position =posi;
	rangeMesh.material = new BABYLON.StandardMaterial('mat', scene);
	rangeMesh.material.diffuseColor = BABYLON.Color3.White();
	rangeMesh.material.alpha = 0.5;
        rangeMesh.visibility = 1;
        return [viewMesh, rangeMesh];
    }

    let resetCannon = function() {
        // メッシュは一旦全部削除, 改めてメッシュを作成
        let cannonInfoList2 = [];
        for (let cinfo of cannonInfoList) {
            let [ix,iz] = [cinfo.ix, cinfo.iz];
            cinfo.mesh.dispose(); cinfo.mesh = null;
            cinfo.range.dispose(); cinfo.range = null;
            if (maze.data_[iz][ix]) { // 壁
                let [x,z] = ixz2xz(cinfo.ix, cinfo.iz);
                let posi = new BABYLON.Vector3(x, wH, z);
                let [viewMesh,rangeMesh] = createCannonMesh(posi);
                // let key = cinfo.key;
                // let vit = cinfo.vit+10;
                // cannonInfoList2.push({key:key, ix:ix, iz:iz, mesh:viewMesh, range:rangeMesh, vit:vit});
                cinfo.mesh = viewMesh;
                cinfo.range = rangeMesh;
                let vit = cinfo.vit+10;
                cannonInfoList2.push(cinfo);
            } else { // 通路
                gamePoint += cinfo.vit; // 残vitをポイント還元
            }
        }
        cannonInfoList = cannonInfoList2;
console.log("\ncannonInfo.size=",cannonInfoList.length, ", gamePoint=",gamePoint);
    }
    ...
    let clearStage = function() {
        ...
        nAgents = maxAgents;
        needClearPoint = Math.floor(maxAgents*0.8);
        let meshes = createStaticMesh(scene);
        resetAllAgent(meshes, scene);
        resetCannon();
        setDest();
    ...
    scene.onBeforeRenderObservable.add(() => {
        ...
        cannonInfoList = cannonInfoList2;
        while (cannonInfoList_.length > 0) {
            let cinfo = cannonInfoList_.pop();
            cannonInfoList.push(cinfo);
        }
        if (needClearPoint <= 0) {
            clearStage();
        }
