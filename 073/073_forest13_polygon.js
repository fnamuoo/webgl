// 「Quick Tree Generator」で森をつくる（激重）
//
// キャラクターコントローラー
//  - カーソル上/(w) .. 前進
//  - カーソル下/(s) .. 後進／降下（空中）
...
var createScene = function () {
    ...
    woodMaterial.diffuseTexture = woodTexture = new BABYLON.WoodProceduralTexture("woodtexture", 512, scene);
    ...
    // 微妙に位置がずれるので補正
    function yposi2(x,z) {
        return yposi(x,z)-1.2;
    }

    let treeSubOrg = QuickTreeGenerator(5, 3, 1, woodMaterial, leafMaterial);
    x = size_, z = size_;
    treeSubOrg.position = new BABYLON.Vector3(x,yposi2(x,z),z);

    let nMesh = 10000, rng = size_, rngLimit = size_*0.2;
    while (nMesh > 0) {
        ...
        treeSub.position = new BABYLON.Vector3(x,yposi2(x,z)+sTrunk/2,z);
