...
function rc2idir(v1,v2) {
  let q = [v2[0]-v1[0], v2[1]-1[1], v2[2]-1[2]];
  let key = q.join(",");
  return IDIRrv[key];
}


export class MazeData04 {
  ...
  create_from(sdata) {
    ...
    this.ny_1 = this.ny_-1;
    this.nz_1 = this.nz_-1;
console.log("nx,y,z=",[this.nx_,this.ny_,this.nz_]);
    this.init(0);
    this.pGoalList_ = [];
    for (let iy = 0; iy < this.ny_; ++iy) {
      let splane = sdata.shift();
      for (let iz = 0; iz < this.nz_; ++iz) {
        let sline = splane.shift();
  ...
  create_dig_20() {
      ...
      // let iilist = [0,0,1,1,2,3,3,4,4,5]; // 水平移動の確率を2倍に
  ...
  setStartGoal(s = [null, null, null], g = [null, null, null]) {
  ...
  idx2xyz(index) {
      let iy = Math.floor(index / this.nxz_);
      let v = index - iy*this.nxz_;
      let iz = Math.floor(v / this.nx_);
      let ix = v - iz*this.nx_;
    return [ix, iy, iz];
  }
        ...
        for (let ix = 0; ix < this.nx_; ++ix) {
          ...
          if (eqv3(this.pStart_, p)) {
            ...
            if (this.data_[ix][iy][iz] >= 1000) {
              ...
              let v4 = parseInt(Math.floor(v/1000));
              let v3 = parseInt(Math.floor((v-v4*1000)/100));
              let v2 = parseInt(Math.floor((v-v4*1000-v3*100)/10));
              let v1 = v%10;
              if (v4 == 1) {
            ...
            } else if (((iy > 0) && (iy < this.ny_)) && (this.data_[ix][iy-1][iz] == -1) && (this.data_[ix][iy+1][iz] == 0)) {
              // 垂直抗の底
              str += "[_";
            } else if (((iy > 0) && (iy < this.ny_)) && (this.data_[ix][iy-1][iz] == 0) && (this.data_[ix][iy+1][iz] == 0)) {
              // 垂直抗の中間
              str += "[-";
            } else if (((iy > 0) && (iy < this.ny_)) && (this.data_[ix][iy-1][iz] == 0) && (this.data_[ix][iy+1][iz] == -1)) {
              // 垂直抗の上（天井
              str += "[`";
            } else {
              str += "  ";
