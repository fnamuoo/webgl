...
// 7: Maze4(立体迷路：通路・壁が同じ大きさ)
// 8: Maze4(立体迷路：最短経路のみ：通路・壁が同じ大きさ)
let mapType = 8;
let nmapType = 8;
...
let map7mapdivIni = [9, 7, 9];
let map7mapdiv = [9, 7, 9];
let map8mapdivIni = [9, 7, 9];
let map8mapdiv = [9, 7, 9];
...
function initMapPara() {
    ...
    map6imap = 0;
    map7mapdiv = JSON.parse(JSON.stringify(map7mapdivIni));
    map8mapdiv = JSON.parse(JSON.stringify(map8mapdivIni));
}
...
window.onload = () => {
    ...
    function createWorld() {
        ...
        if (mapType == 7) {
            // Maze4(通路・壁が同じ大きさの迷路) を使った 3D迷路
            function createBox(sx, sy, sz, px, py, pz, adjx, adjy, adjz, viMaterial) {
                const sx_ = sx/2, sy_ = sy/2, sz_ = sz/2;
                const moWall2Body = new CANNON.Body
                ({mass: 0,
                  shape: new CANNON.Box(new CANNON.Vec3(sx_, sy_, sz_)),
                ...
                world.addBody(moWall2Body);
                const viWall2Geo = new THREE.BoxGeometry(sx, sy, sz);
                const viWall2Mesh = new THREE.Mesh(viWall2Geo, viMaterial);
                scene.add(viWall2Mesh);
                viWall2Mesh.position.copy(moWall2Body.position);
                viWall2Mesh.quaternion.copy(moWall2Body.quaternion);
            }
            function createSlope(sx, sy, sz, px, py, pz, adjx, adjy, adjz, quat, viMaterial) {
                const sx_ = sx/2, sy_ = sy/2, sz_ = sz/2;
                const moWall2Body = new CANNON.Body
                ({mass: 0,
                  shape: new CANNON.Box(new CANNON.Vec3(sx_, sy_, sz_)),
            ...
            // mzdata.create_from(Maze4data.get_map_data("sdata00"))  // 撮影用
            // mzdata.create_from(Maze4data.get_map_data("sdata10"))
            // mzdata.create_from(Maze4data.get_map_data("sdata11"))
            // mzdata.create_from(Maze4data.get_map_data("sdata15"))
            // mzdata.create_from(Maze4data.get_map_data("sdata20"))  // 撮影用
            // mzdata.create_from(Maze4data.get_map_data("sdata26"))
            // mzdata.create_from(Maze4data.get_map_data("sdata27"))  // 撮影用
            mzdata.seekPath2(); // 最短経路をもとめる
            mzdata.dbgPrintMap();
            nx = mzdata.nx_, ny = mzdata.ny_, nz = mzdata.nz_;
            console.log("size=",[nx,ny,nz]);
            const moMazeMtr = new CANNON.Material({name: 'maze'});
            const blockSize = 16, blockSize_ = blockSize/2;
            const blockSizeH = 4, blockSizeH_ = blockSizeH/2;
            let adjx = blockSize_*1.5, adjy = 0, adjz = blockSize_*1.5;
            let ix, iy = 0, iz;
            let mateShortPath1 = new THREE.MeshBasicMaterial({color: "#008000"});
            let mateShortPath2 = new THREE.MeshBasicMaterial({color: "#00c000"});
            let mateFloor1 = new THREE.MeshBasicMaterial({color: "#808080"});
            let mateFloor2 = new THREE.MeshBasicMaterial({color: "#a0a0a0"});
            let mateWall1 = new THREE.MeshBasicMaterial({color: "#ffffff"});
            let mateWall2 = new THREE.MeshBasicMaterial({color: "#efefef"});
            let mateSlope = new THREE.MeshBasicMaterial({color: "#a0a0a0"});
            let mateSlopeSP3 = new THREE.MeshBasicMaterial({color: "#00a000"});
            for (let iiy = 0; iiy < ny-1; ++iiy) {
                iy = iiy * blockSizeH;
                for (let iiz = 0; iiz < nz; ++iiz) {
                    iz = iiz * blockSize;
                    for (let iix = 0; iix < nx; ++iix) {
                        ix = iix * blockSize;
                        let geoID = mzdata.data_[iix][iiy][iiz];
                        if (geoID == -1) {
                            // 壁
                            const sx = blockSize, sy = blockSizeH, sz = blockSize;
                            const sx_ = sx/2, sy_ = sy/2, sz_ = sz/2;
                            const px = ix, py = iy, pz = iz;
                            {
                                let viWall2Mtr;
                                if ((iiy%2) == 0) {
                                    // 床／天井
                                    if (mzdata.path2_.includes(mzdata.xyz2idx([iix,iiy+1,iiz]))) {
                                        // １つ上(iiy+1)が最短経路
                                        if (((iix+iiy+iiz)%2) == 0) {
                                            viWall2Mtr = mateShortPath1;
                                        } else {
                                            viWall2Mtr = mateShortPath2;
                                        }
                                    } else {
                                        // 普通の床
                                        if (((iix+iiy+iiz)%2) == 0) {
                                            viWall2Mtr = mateFloor1;
                                        } else {
                                            viWall2Mtr = mateFloor2;
                                        }
                                    }
                                } else {
                                    // 壁
                                    if (((iix+iiy+iiz)%2) == 0) {
                                        viWall2Mtr = mateWall1;
                                    } else {
                                        viWall2Mtr = mateWall2;
                                    }
                                }
                                createBox(sx, sy, sz, px, py, pz, adjx, adjy, adjz, viWall2Mtr);
                            }
                        } else if (geoID >= 1000) {
                            // 斜抗
                            let v4 = parseInt(Math.floor(geoID/1000));
                            let v3 = parseInt(Math.floor((geoID-v4*1000)/100));
                            let v2 = parseInt(Math.floor((geoID-v4*1000-v3*100)/10));
                            let v1 = geoID % 10;
                            let vimateSlope = mateSlope;
                            if (v4 == 1) {
                                // 斜抗（１）
                                if (v2==1 && v1==0) {
                                    for (let ii = 0; ii < 3; ++ii) {
                                        if (mzdata.path2_.includes(mzdata.xyz2idx([iix,iiy+ii,iiz]))) {
                                            vimateSlope = mateSlopeSP3;
                                            break;
                                        }
                                    }
                                }
                                if (geoID == 1010) { // N0
                                    const len = Math.sqrt(blockSize**2 + (2*blockSizeH)**2);
                                    const sx = blockSize, sy = 0.1, sz = len;
                                    const sx_ = sx/2, sy_ = sy/2, sz_ = sz/2;
                                    let ang = Math.atan((2*blockSizeH)/blockSize);
                                    const px = ix, py = iy+blockSizeH_, pz = iz;
                                    var quat = new CANNON.Quaternion().setFromAxisAngle(new CANNON.Vec3(1, 0, 0), ang);
                                    createSlope(sx, sy, sz, px, py, pz, adjx, adjy, adjz, quat, vimateSlope);
                                } else if (geoID == 1110) { // W0
                                    const len = Math.sqrt(blockSize**2 + (2*blockSizeH)**2);
                                    const sx = len, sy = 0.1, sz = blockSize;
                                    const sx_ = sx/2, sy_ = sy/2, sz_ = sz/2;
                                    let ang = Math.atan((2*blockSizeH)/blockSize);
                                    const px = ix, py = iy+blockSizeH_, pz = iz;
                                    var quat = new CANNON.Quaternion().setFromAxisAngle(new CANNON.Vec3(0, 0, -1), ang);
                                    createSlope(sx, sy, sz, px, py, pz, adjx, adjy, adjz, quat, vimateSlope);
                                } else if (geoID == 1210) { // S0
                                    const len = Math.sqrt(blockSize**2 + (2*blockSizeH)**2);
                                    const sx = blockSize, sy = 0.1, sz = len;
                                    const sx_ = sx/2, sy_ = sy/2, sz_ = sz/2;
                                    let ang = Math.atan((2*blockSizeH)/blockSize);
                                ...
                                if (geoID == 2010) { // N0
                                    const len = Math.sqrt((3*blockSize)**2 + (2*blockSizeH)**2);
                                    const sx = blockSize, sy = 0.1, sz = len;
                                    const sx_ = sx/2, sy_ = sy/2, sz_ = sz/2;
                                    let ang = Math.atan((2*blockSizeH)/(3*blockSize));
                                    const px = ix, py = iy+blockSizeH_, pz = iz+-blockSize;
                                    var quat = new CANNON.Quaternion().setFromAxisAngle(new CANNON.Vec3(1, 0, 0), ang);
                                    createSlope(sx, sy, sz, px, py, pz, adjx, adjy, adjz, quat, vimateSlope);
                                } else if (geoID == 2110) { // W0
                                    const len = Math.sqrt((3*blockSize)**2 + (2*blockSizeH)**2);
                                    const sx = len, sy = 0.1, sz = blockSize;
                                    const sx_ = sx/2, sy_ = sy/2, sz_ = sz/2;
                                    let ang = Math.atan((2*blockSizeH)/(3*blockSize));
                                    const px = ix-blockSize, py = iy+blockSizeH_, pz = iz;
                                    var quat = new CANNON.Quaternion().setFromAxisAngle(new CANNON.Vec3(0, 0, -1), ang);
                                    createSlope(sx, sy, sz, px, py, pz, adjx, adjy, adjz, quat, vimateSlope);
                                } else if (geoID == 2210) { // S0
                                    const len = Math.sqrt((3*blockSize)**2 + (2*blockSizeH)**2);
                                    const sx = blockSize, sy = 0.1, sz = len;
                                    const sx_ = sx/2, sy_ = sy/2, sz_ = sz/2;
                                    let ang = Math.atan((2*blockSizeH)/(3*blockSize));
            ...
            }
            {
                // 最短経路を線で表示
                let adjx = blockSize_*1.5, adjy = 0, adjz = blockSize_*1.5;
                let idxlist = mzdata.path2_;
                let nidxlist = idxlist.length;
                const viObj1Points = [];
                let p1 = mzdata.idx2xyz(idxlist[0]);
                for (let ii = 0; ii < nidxlist; ++ii) {
                    let p2 = mzdata.idx2xyz(idxlist[ii]);
                    if ((p1[0] == p2[0]) && (p1[2] == p2[2]) && (Math.abs(p1[1]-p2[1])==1)) {
                        p1 = p2;
                        continue;
                    }
                    p1 = p2;
                    let pv2 = [p2[0]*blockSize + adjx,
                               p2[1]*blockSizeH + adjy,
                               p2[2]*blockSize + adjz];
                    viObj1Points.push( new THREE.Vector3(pv2[0],pv2[1],pv2[2]) );
                }
                {
            ...
            // 車の位置をスタート地点に
            let sp = mzdata.pStart_;
            iniX = sp[0]*blockSize +adjx;
            iniY = sp[1]*blockSizeH+adjy;
            iniZ = sp[2]*blockSize +adjz;
            // 車の向きの最短経路のある方向に向ける
            const idir2rotY = {3:0, 4:0.5, 0:1, 1:-0.5};
            let bhit = false;
            for (let iydiff of [0, 1, 2, -1, -2]) {
                for (let idir of [3,4,0,1]) {
                    if (mzdata.isWallPosiDir(sp, idir)) {
                        continue;
                    }
                    let p = [sp[0] + Maze4.IDIR[idir][0],
                             sp[1] + Maze4.IDIR[idir][1] + iydiff,
                             sp[2] + Maze4.IDIR[idir][2]];
                    let idx = mzdata.xyz2idx(p);
                    if (mzdata.path2_.includes(idx)) {
                        iniRotY = idir2rotY[idir];
                        bhit = true;
                        break;
            ...
            if (0) {
                let sp = mzdata.pStart_;
                const px = sp[0]*blockSize+adjx, py = sp[1]*blockSizeH+adjy, pz = sp[2]*blockSize+adjz;
                let radius = 1.5; // 3;
                const moBallBody = new CANNON.Body
                ({mass: 1,
                    shape: new CANNON.Sphere(radius),
        ...
        if (mapType == 8) {
            // Maze4(通路・壁が同じ大きさの迷路) を使った 経路（最短経路のみ）
            function createBox(sx, sy, sz, px, py, pz, adjx, adjy, adjz, viMaterial) {
                const sx_ = sx/2, sy_ = sy/2, sz_ = sz/2;
                const moWall2Body = new CANNON.Body
                ({mass: 0,
            ...
            let mateSlope = new THREE.MeshBasicMaterial({color: "#a0a0a0"});
            let mateSlopeSP3 = new THREE.MeshBasicMaterial({color: "#00a000"});
            let ny_1 = ny-1, ny_2 = ny_1-1, ny_h = Math.floor(ny/2), crate = 0.8;
            for (let iiy = 0; iiy < ny_1; ++iiy) {
                iy = iiy * blockSizeH;
                let cR1 = iiy/ny_2, cG1 = Math.abs(iiy-ny_h)/ny_2, cB1 = (ny_2-iiy)/ny_2;
                let cAve = (cR1 + cG1 + cB1)/3;
                cR1 = Math.max(cR1, cAve);
                cG1 = Math.max(cG1, cAve);
                cB1 = Math.max(cB1, cAve);
                let cR2 = cR1*crate, cG2 = cG1*crate, cB2 = cB1*crate;
                mateShortPath1 = new THREE.MeshBasicMaterial({color: new THREE.Color(cR1,cG1,cB1)});
                mateShortPath2 = new THREE.MeshBasicMaterial({color: new THREE.Color(cR2,cG2,cB2)});
                mateSlopeSP3 = new THREE.MeshBasicMaterial({color: new THREE.Color(cAve,cAve,cAve)});
                ...
                for (let iiz = 0; iiz < nz; ++iiz) {
                    ...
                    for (let iix = 0; iix < nx; ++iix) {
                        ...
                        if (geoID == -1) {
                            ...
                            // const sx_ = sx/2, sy_ = sy/2, sz_ = sz/2;
                            // const px = ix, py = iy, pz = iz;
                            const sx = blockSize, sy = 0.1, sz = blockSize;
                            const sx_ = sx/2, sy_ = sy/2, sz_ = sz/2;
                            const px = ix, py = iy+blockSizeH_, pz = iz;
                            // let bShortPathFloor = false;
                            {
                                let viWall2Mtr;
                                if ((iiy%2) == 0) {
                                    // 床／天井
        ...
        if (gamepad.enable_) {
            ...
            if (keyChangeCool > 0) {
                ...
                if (gamepad.para_.btnBK) {
                    keyChangeCool = 30;
                    mapType = ((mapType+nmapType-1) % nmapType) + 1;
                    initMapPara();
                    [world, camera, scene, renderer, orbitControls, vehicle] = createWorld();
                    resetVehiclePosi();
                }
