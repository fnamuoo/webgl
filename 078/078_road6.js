// 凹凸のある道路をキャラクターコントローラーでCNSと競争する(6)
//
// キャラクターコントローラー
//  - カーソル上下/(w,s) .. 前進/後進
//  - カーソル右左/(a,d) .. 方向転換
...
var createScene = function () {
    ...
    let stageLv=1;
    let pStart, pGoal, vStart, meshGoal = null, goalRank;
    ...
    function createStaticMesh(scene) {
        ...
        if (1) {
            ...
            pStart = plist2[2].clone(); // [1]のギリギリだと落ちることがあるので余裕を持たせる
            pGoal = plist2[plist2.length-3].clone();
    ...
    let addAgent = function() {
        ...
        if (agentCubeList.length == 0) {
            ...
            agentParams.maxAcceleration = 5.8; agentParams.maxSpeed = 3.7
        }
    ...
    const R90 = Math.PI/2, R180 = Math.PI;

    let resetPosition = function() {
        characterController._position = pStart.clone();
        characterController._position.y += h*2;
        // 開始時のキャラクターコントローラーの向きを道路の向きにあわせる
        let qrot;
        if (Math.abs(vStart.x) > Math.abs(vStart.z)) {
            if (vStart.x > 0) {
                qrot = BABYLON.Quaternion.FromEulerAngles(0, R90, 0);
            } else {
                qrot = BABYLON.Quaternion.FromEulerAngles(0, -R90, 0);
            }
        } else {
            if (vStart.z > 0) {
                qrot = BABYLON.Quaternion.FromEulerAngles(0, 0, 0);
            } else {
                qrot = BABYLON.Quaternion.FromEulerAngles(0, R180, 0);
            }
        }
        displayCapsule.rotationQuaternion = qrot;
        characterOrientation = qrot;
    }
    ...
    let nextStage = function() {
        ...
        iCheckGoal = 199; // 誤動作するので、時間差で「ゴール順位情報」を再設定する
    }
