...
var createScene = function () {
    ...
    // Player/Ball
    let myMesh = BABYLON.MeshBuilder.CreateSphere("target", { diameter: 1.2, segments: 8 }, scene);
    let myAgg = new BABYLON.PhysicsAggregate(myMesh, BABYLON.PhysicsShapeType.SPHERE, { mass: 1, friction: 0.9, restitution:0.05}, scene);
    ...
    let cooltime_act = 0, cooltime_actIni = 30, vforceBase = 30, vforce = 30, bDash=3;
    ...
    scene.registerAfterRender(function() {
        ...
        if (map["Shift"]) {
            vforce = vforceBase * bDash;
        }
        if (map[" "]) {
            let vdir = new BABYLON.Vector3(0 ,vforce*3, 0);
