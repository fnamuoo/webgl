...
*/
    ...
    let scale = 1 / MAX * 4 ; // 周波数みたいなもの。大きくすると文様が細かく、小さくすると文様が大きくなる
    let funcColor = (px, py, time) => {
        let x = px * scale;
        let y = py * scale;
        // let color = Math.abs((noise.perlin2(x, y)+1)/2);
        let color = Math.abs((noise.perlin3(x, y, time)+1)/2);
        return {
            r: color,
            g: color, // * 0.5,
